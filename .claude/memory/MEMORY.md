...
- [delivery run-sheet](delivery_run_sheet.md) — **STARTED 2026-06-23:** process-delivery.md restructured into a 6-step→gate run-sheet mirroring the assessment one; the 3-layer gate architecture decided (format doc = source of truth for skill AND linter; deterministic linter; agent only for residual judgment); **Step 1 (cluster spec / delivery frame) BUILT + back-tested — all 3 S1 clusters PASS, definition-phase gate met**. NEXT: build the deterministic spine (Step 3 validate-delivery-coverage, then Step 6 validate-delivery-plan); steps 2/4/5 stay human.
