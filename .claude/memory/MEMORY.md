...
- [delivery run-sheet](delivery_run_sheet.md) — **IN PROGRESS 2026-06-24:** 6-step→gate run-sheet (docs/process-delivery.md) mirroring the assessment one; 3-layer gate architecture (format doc = source of truth for skill AND linter). **BUILT + back-tested on CL1:** Step 1 (cluster spec — all 3 clusters, definition gate met), Step 3 (coverage spine 90/90 — tags standardised project-wide), Step 4 slide-plan gate (14/14 — slide plan now kept+validated; 14 reverse-engineered from the decks). **DECIDED:** Step-4 image model (mandatory `image:` field; generated→placed in-pipeline, reuse→human paste), Step-5 practice model (assessment decomposed 1:1 into interleaved practice steps), steps 2+3 merge. **NEXT:** draw.io diagram skill (Tim's request) + his image-gen import → wire the deck-image pipeline; then Step 6 validate-delivery-plan.
