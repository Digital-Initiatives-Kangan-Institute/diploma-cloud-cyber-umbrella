...
Stdlib only. Reads a run-sheet doc written in the umbrella's step->gate convention
...
def load_inventory(skills_root: Path, engines_root: Path) -> dict:
    ...
    # the course-agnostic engine scripts (umbrella scripts/, one level deep)
...
WEBSITE_HINT = re.compile(r"\.astro|states\.ts|/src/|website|intranet", re.I)
...
def mermaid(model: dict) -> str:
    ...
    for s in steps:
        sub = []
        # 🏠 umbrella lane — the primary skill + every tool the stage runs
        umbrella = ([s["skill"]] if s["skill"] else []) + [t["name"] for t in s.get("tools", [])]
        if umbrella:
            sub.append(f"<i>🏠 {_lbl(', '.join(umbrella))}</i>")
...
def tooling_table(model: dict) -> str:
    rows = ["| Step | 🏠 Umbrella tooling (skill 🛠 · agent 🤖 · engine ⚙ · MCP 🔌) | 📦/🌐 Sub-repo data drawn on | Actor |",
            "|---|---|---|---|"]
    for s in model["steps"]:
        tools = []
        if s["skill"]:
...
def render_md(model: dict) -> str:
    return "\n\n".join([
        ...
        tooling_table(model),
        "## Legend",
        "- **Grey step** — a unit of work. Locus sub-lines: **🏠** the umbrella tooling it runs "
        "(skill/engine/agent), **📦/🌐** the sub-repo / website data it draws on, **👤** if human-led, "
        "then loop scope.\n"
...
def main() -> None:
    ...
    ap.add_argument("--engines-root", type=Path, default=Path("scripts"),
                    help="root holding the course-agnostic engine scripts (default: scripts)")
