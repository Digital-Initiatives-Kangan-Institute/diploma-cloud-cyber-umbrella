...
description: |
  ...
  the umbrella's convention (`**N · `skill` — Title** *(scope)*` step headers;
  ...
  → done, gates coloured by build status), a tooling-&-locus table (🏠 umbrella
  tools each stage runs · 📦/🌐 sub-repo data it draws on · 👤 human-led), an
  audit table (named-but-missing validators, stale doc status markers), and a
  machine-readable JSON model. Course-agnostic: point it at any factory's
...
Map the process a **factory** (a repo authored to the umbrella's step→gate convention) currently
...
2. **What runs at each stage, and where?** — the skills / agents / engines / MCP each step uses
   (🏠 umbrella tooling) and the sub-repo / website data it draws on (📦 / 🌐), plus which stages are
   👤 human-led.
...
Stdlib only — run with any Python 3 (no venv). From the umbrella root:

```bash
python3 .claude/skills/map-process/map_process.py \
    docs/process-assessment.md docs/process-delivery.md \
...
- `--engines-root` (default `scripts`) — the course-agnostic engine scripts (the umbrella `scripts/`,
  scanned one level deep) — so `build_topic_deck`, `generate_mapping_doc`, `kangan_deck` etc. resolve.
...
A run-sheet written the umbrella way (see `docs/process-delivery.md` / `docs/process-assessment.md`):
...
`build_topic_deck` (an `--engines-root` script). `mcp__*` tokens are recognised as MCP tools.

## Locus (where each stage runs / draws data)

Per your factory's architecture the map tags each stage's loci inline:
- **🏠 umbrella** — the tooling (skills/engines/agents) resolved under the skills + engines roots.
- **📦 sub-repo / 🌐 website** — data artefacts (`.md`/`.docx`/`.pptx`/`.yaml`/`.ts`/…) the stage draws
  on; a token that looks website-side (`.astro`, `states.ts`, `src/`, `intranet`) is tagged 🌐, else 📦.
- **👤 human-led** — a step with no primary automated skill.

This surfaces the crossing you care about — an umbrella tool operating on a sub-repo data file.
