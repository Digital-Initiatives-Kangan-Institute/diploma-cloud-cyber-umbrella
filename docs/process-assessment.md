...
[scenario-flow.md](scenario-flow.md) · [website-architecture.md](website-architecture.md) · detail [§6](#6--scenario-materials).
> **⟱ Gate 6→7:** *human review* — every scenario file in the checklist is authored, marked carry-over,
> or flagged **TBD**; AC items the scenario must satisfy are covered. *(No script — the scenario-plan linter
> + cross-check against the consolidated `SR-*` register are the planned validators here.)*
