...
Step 5 is **per scenario** (once every cluster has reached step 4); steps 7–8 loop **per AT**; steps 9–10
are **cluster-level** (after all ATs exist).
...
> provides those `SR-*` is checked at the scenario cross-check, once it is built — see step 6.)*
...
the scenario plan is validated against; it is **derived — never hand-edited** (the per-cluster plans stay the
source). Generated by `generate_consolidated_plan.py`. → detail [§5](#5--consolidate-the-assessment-plans).
...
**6 · Scenario materials**
Author the shared cluster scenario **from the agreed design (step 4)** — the in-world content the ATs
depend on, single-sourced on the website (in-world only — no course/assessment meta-language) — and capture
the **scenario plan** (`scenario-plans/<SEMESTER>.md`, to the format standard): the in-world **elements**
(`SE-NN`), each binding the `SR-*` it provides. →
[scenario-plan-format.md](scenario-plan-format.md) · [scenario-flow.md](scenario-flow.md) · [website-architecture.md](website-architecture.md) · detail [§6](#6--scenario-materials).
> **⟱ Gate 6→7:** *validator* `validate-scenario-plan` = **PASS** — the scenario plan conforms to the format
> and every `SR-*` in the consolidated assessment plan is satisfied by ≥1 scenario element, with no phantom
> `SR-*`; **+ human review** — the world is coherent and believable (no leakage between clusters; consistent
> system state) and the scenario checklist is authored / carry-over / **TBD**.

**7 · Assessor instruments** *(loop per AT)*
...
map closes the loop). → [cluster-authoring-conventions.md](cluster-authoring-conventions.md) · [document-template-system.md](document-template-system.md) · detail [§7](#7--assessor-instruments).
> **⟱ Gate 7→8:** *validator* `validate-at-traceability` = **PASS** for the AT (no phantom tags, no
> free-floating criteria; pass the AT's allocation via `--expect` for reverse-coverage) **+ human review**.

**8 · Student instruments** *(loop per AT)*
Derive the student-facing instrument from the assessor version (shared Details/Task/Resources; strip the
Marking Guide, benchmark, and model answers). → detail [§8](#8--student-instruments).
> **⟱ Gate 8→9:** *human review* — student copy carries everything the student needs and nothing
> assessor-only. *(No script.)*

**9 · Mapping documents** *(cluster-level)*
...
[mapping-document-standard.md](mapping-document-standard.md) · detail [§9](#9--mapping-documents).
> **⟱ Gate 9→10:** *validator* `validate-mapping-doc` = **PASS** for every unit — complete vs the unit's
> own UoC (every item present + mapped) and accurate vs the benchmarks (PC/PE/KE hard; FS/AC advisory)
> **+ human review**.

**10 · Cluster coverage** *(capstone)*
Confirm the cluster's ATs, taken together, evidence **every** consolidated item. → [validate-cluster-coverage skill](../diploma-cloud-cyber-content/.claude/skills/validate-cluster-coverage/SKILL.md) · detail [§10](#10--cluster-coverage).
> **⟱ Gate 10→11:** *validator* `validate-cluster-coverage` = **100%** (no MISSING required item, no
> PHANTOM reference; AC environment-satisfied unless `--include-ac`) **+ human review**.

**11 · Institutional pre-validation**
Run the institutional Pre-Validation Tool over each AT and address findings. → detail [§11](#11--institutional-pre-validation).
> **⟱ Gate 11→done:** *human / institutional* sign-off. The cluster's assessment is complete.
...
- **Scenario design (the narrative)** — discuss and arrive at the story, organisation, stakeholders,
  ...
  step-6 materials are authored from and the instruments reference.
...
Capture the **scenario plan** (`scenario-plans/<SEMESTER>.md`) to the format standard
([scenario-plan-format.md](scenario-plan-format.md)): one per scenario, it is the **authored** bridge from
the `SR-*` contract to the world — each in-world **element** (`SE-NN`) names where it is single-sourced and
declares, on a `Satisfies:` line, the `SR-*` it provides. **`validate-scenario-plan`** lints the format and
cross-checks bidirectionally against the consolidated assessment plan's register: every `SR-*` is satisfied
by ≥1 element (UNCOVERED otherwise), no element claims an `SR-*` outside the register (PHANTOM otherwise).
The consolidated plan is **derived**; the scenario plan is **authored** — the two are the contract and the
world that meets it.
