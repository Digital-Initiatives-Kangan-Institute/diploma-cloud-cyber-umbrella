...
- **CL1** — **mappings** now generated by the engine and validate clean, but **instruments not yet
  benchmark-conformant**. CL1 predates the machine-readable-benchmark discipline, so it has no invertible
  benchmark; the engine drives it from a direct inversion synthesised from its hand-authored `DATA_*` (the
  `source: "data"` path in `CLUSTERS`). Regenerating fixed the structural artefacts (the 401 AC block-row,
  the 517 PE split) and several verbatim defects (a PC typo, a truncation, a bloated FS table). Full
  conformance — real machine-readable, UoC-tagged benchmarks in CL1's three assessor instruments, so
  `validate-at-traceability` can check them — remains a separately-scoped retrofit.
