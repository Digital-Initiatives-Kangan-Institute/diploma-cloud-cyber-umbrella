...
**Generate.** The mapping generator reads the source UoC for the rows, inverts the assessor benchmarks
for the AT-column codes, applies the FS/AC closest-fit tables, and fills the institutional template. The
current reference implementations are the per-cluster `scripts/s1_clN/build_s1_clN_mapping_docs.py`
generators (CL2/CL3 are the contract-conformant pattern; run with `--dump` to review the inversion, then
`--build`). Output: `<cluster>/mappings/<UNIT>_Assessment_Mapping.docx`.
...
- **CL2, CL3** — contract-conformant: machine-readable per-criterion benchmarks, generated mappings,
  validate clean (completeness + PC/PE/KE accuracy).
- **CL1** — the legacy outlier. It predates the machine-readable-benchmark discipline: its mappings were
  hand-authored into `DATA_*` dicts and edited into the docx in place, with no invertible benchmark and
  some structural artefacts (a leftover AC context row; a Performance-Evidence parent bullet split into
  per-sub-bullet rows). Bringing CL1 onto this standard means giving its assessors machine-readable
  benchmarks, then regenerating its mappings through the conformant generator and validating the result.
